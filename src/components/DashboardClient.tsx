...
import { createWedding, updateWedding, deleteWedding, signOut } from '@/app/actions'
...
function WeddingForm({ mode, initial, onClose }: WeddingFormProps) {
  const action = mode === 'create' ? createWedding : updateWedding
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
        <h3 className="text-lg font-semibold mb-5">
          {mode === 'create' ? 'Crear evento de boda' : 'Editar boda'}
        </h3>
        <form action={formAction} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              type="text"
              required
              defaultValue={initial?.couple_names}
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl text-center">
        ...
        <h3 className="text-lg font-semibold mb-2">¿Eliminar esta boda?</h3>
        <p className="text-gray-500 text-sm mb-1">
          <strong>{wedding.couple_names}</strong>
        </p>
        <p className="text-gray-400 text-xs mb-5">
          Se eliminarán todas las fotos asociadas. Esta acción no se puede deshacer.
        </p>
        {error && (
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={isPending}
            className="flex-1 bg-red-500 hover:bg-red-600 text-white py-2.5 rounded-xl text-sm font-medium disabled:opacity-50 transition"
          >
...
export default function DashboardClient({
  ...
  const [modal, setModal] = useState<
    | { type: 'create' }
    | { type: 'edit'; wedding: Wedding }
    | { type: 'delete'; wedding: Wedding }
    | null
  >(null)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 to-pink-50">
              ...
              >
                ...
                <div className="flex justify-between items-start gap-2 mb-1">
                  ...
                  <div className="flex gap-1 shrink-0">
                    <button
                      onClick={() => setModal({ type: 'edit', wedding })}
                    ...
                    <button
                      onClick={() => setModal({ type: 'delete', wedding })}
                ...
                <p className="text-gray-500 text-sm">{wedding.name}</p>
                {wedding.date && (
      ...
      {modal?.type === 'create' && (
        <WeddingForm mode="create" onClose={() => setModal(null)} />
      )}
      {modal?.type === 'edit' && (
        <WeddingForm mode="edit" initial={modal.wedding} onClose={() => setModal(null)} />
      )}
      {modal?.type === 'delete' && (
        <DeleteConfirm wedding={modal.wedding} onClose={() => setModal(null)} />
      )}
    </div>
  )
}
